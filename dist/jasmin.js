...
jasmin.ResponseManager.prototype.parseResponse = function(a, b, c, d) {
  ...
  if (void 0 !== this.activeResponses[a]) {
    ...
    this.y = this.x = void 0;
    -1 !== this.pointerDeviceEventsList.indexOf(a) && (this.x = b.pageX, this.y = b.pageY, f = !0);
  }
  this.critical = f;
  this.mode = a;
  this.id = c;
    ...
    for (b in this.pointerDeviceEventsList) {
      if (a = this.pointerDeviceEventsList[b], void 0 !== this.activeResponses[a]) {
        ...
        for (var c in this.activeResponses[a].buttons) {
          report("ResponseManager.unbind", a), $(c).unbind(a);
        }
      }
    }
  }
...
jasmin.ResponseManager.prototype.updateResponseLog = function() {
  this.responseLog = {na:this.name, cr:this.critical, mo:this.mode, id:this.id, la:this.label, ti:this.time, x:this.x, y:this.y};
};
...
jasmin.Slideshow = function(a, b, c, d, e, f) {
  this.eventManager = b;
  this.translator = f;
  this.target = a;
  ...
  void 0 === f ? (this.translator = {}, this.translator.translate = function(a) {
    return a;
  }) : this.translator = f;
};
  ...
  if (this.slideCounter >= this.slides.length) {
    ...
    if (0 === this.buttonDelay) {
      ...
      if (this.slideFurthest >= this.slideCounter) {
        ...
        var a = this.translator.translate(this.slides[this.slideCounter]), b = this;
...
jasmin.Slideshow.prototype.showButtons = function() {
  ...
  void 0 !== this.buttonTexts && (a += "<br /><br />" + this.translator.translate(1 === this.slides.length ? this.buttonTexts.only : this.slideCounter === this.slides.length - 1 ? this.buttonTexts.last : 0 === this.slideCounter ? this.buttonTexts.first : this.buttonTexts.middle));
  var b = this;
  this.eventManager.startEvent(-1, function() {
    b.target.html(b.translator.translate(a));
  }, function() {
    b.response();
...
jasmin.Statistics.mean = function(a) {
  ...
  return 0 >= c ? void 0 : b / c;
};
jasmin.Statistics.variance = function(a) {
  var b = jasmin.Statistics.mean(a), c = 0, d = 0, e;
  for (e in a) {
    c += Math.pow(a[e] - b, 2), d++;
  }
  return 1 >= d ? 0 : c / (d - 1);
};
...
jasmin.Statistics.transformZ = function(a, b, c) {
  var d = jasmin.Statistics.similarArray(a), e;
  for (e in a) {
    d[e] = (parseInt(a[e]) - b) / c;
  }
  return d;
};
...
jasmin.Statistics.orderBy = function(a, b) {
  var c = [], d;
  for (d in b) {
    c.push(a[b[d]]);
  }
